...
class LoRALayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        # [Fix 1] 解决 bfloat16 输入与 float32 权重冲突
        # 如果输入 x 是 bfloat16，临时将 LoRA 权重也转为 bfloat16 进行计算
        A = self.lora_A
        B = self.lora_B
...
class LoRALinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        # [Fix 2] 自动对齐输入类型与原始层权重类型
        # 解决 Float32 输入 vs BFloat16 原始层权重的冲突
        if hasattr(self.original_layer, "weight") and x.dtype != self.original_layer.weight.dtype:
            x = x.to(self.original_layer.weight.dtype)
            
        # 注意：正确调用 self.lora(x)，而不是直接访问 self.lora_A
